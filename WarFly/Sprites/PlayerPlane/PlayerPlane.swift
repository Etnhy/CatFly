...
class PlayerPlane: SKSpriteNode {
    ...
    static func populate(at point: CGPoint) -> PlayerPlane {
        ...
        path.addLine(to: CGPoint(x: 87 - offsetX, y: 57 - offsetY))
        path.addLine(to: CGPoint(x: 9 - offsetX, y: 65 - offsetY))
        path.addLine(to: CGPoint(x: 9 - offsetX, y: 75 - offsetY))
        path.closeSubpath()
...
//        playerPlane.physicsBody = SKPhysicsBody(texture: playerPlaneTexture, alphaThreshold: 0.5, size: playerPlane.size)
    ...
    fileprivate func preloadTexturesArrays() {
        for i in 0...2 {
            self.preloadArrays(_stride: animationSpritesStrite[i]) { [unowned self] array in
                switch i {
    ...
    fileprivate func preloadArrays(_stride: (Int,Int, Int), callback: @escaping(_ array: [SKTexture]) -> () ) {
        var array = [SKTexture]()
